...
After preparing your application for an international audience, use the [Angular CLI](cli) to localize your app by performing the following tasks:
...
To localize your application, follow these general steps:
...
6. [Merge translations into the application](#merge).
...
Angular by default uses `en-US` (English in the United States) as your application's source locale.
...
You can change your application's source locale for the build by setting the source locale in the `sourceLocale` field of your application's [workspace configuration](guide/workspace-config "Angular workspace configuration") file (`angular.json`).
The build process (described in [Merge translations into the app](#merge) in this guide) uses your application's `angular.json` file to automatically set the [`LOCALE_ID`](api/core/LOCALE_ID "API reference for LOCALE_ID") token and load the locale data.
...
To translate your application's templates, you need to prepare the text for a translator or translation service by marking text, attributes, and other elements with the Angular `i18n` attribute.
...
Angular 9 uses the source locale configured in your application's [workspace configuration](guide/workspace-config "Angular workspace configuration") file (`angular.json`).
...
If you remove the placeholder, the ICU expression will not appear in your translated application.
...
To merge the completed translations into the application, use the [Angular CLI](guide/glossary#command-line-interface-cli "Definition of CLI") to build a copy of the application's distributable files for each locale.
The build process replaces the original text with translated text, and sets the `LOCALE_ID` token for each distributable copy of the application.
...
ready-to-run application. With Ivy in Angular version 9, AOT is used by default for both
...
Use the `i18n` project option in your application's build configuration file ([`angular.json`](guide/workspace-config "Angular workspace configuration")) to define locales for a project.
...
If `myapp` is the directory containing your application's distributable files, you would typically make available different versions for different locales in locale directories such as `myapp/fr` for the French version and `myapp.com/es` for the Spanish version.
...
For more information on how to deploy applications to a remote server, see [Deployment](guide/deployment "Deployment guide").
...
To manually set an application's source locale to one other than the automatic value, follow these steps:
